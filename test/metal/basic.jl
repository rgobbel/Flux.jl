...
@testset "Basic GPU movement" begin
    ...
    @test gradient(x -> sum(cpu(x)), rand(Float32, 4, 4)) isa Tuple
end
...
@testset "Restructure" begin
    ...
    @test gradient(x -> sum(m̂(θ)(x)), x)[1] isa MtlArray{Float32}
end
...
@testset "Flux.onecold gpu" begin
    ...
    @test Flux.onecold(y) isa MtlArray
    @test y[3, :] isa MtlArray
